...
services.factory("poller", ["$http", "$timeout", "$filter", "$q", function(a, b, c, d) {
    ...
    var url = "http://inteliotstorage.mybluemix.net";
    var f = {};
    ...
    f.checkConnection = function() {
         
        //Determine if the app is able to connect to cloud via GET request
        $.get("http://inteliotstorage.mybluemix.net/api/", function(data) {
            console.log( "startup - reached cloud successfully" );
	    f.init();
       })
       .fail(function() {
            console.log( "startup - fail to reach cloud - OFFLINE mode" );
    ...
    f.init = function() {
        ...
        $.post(url+"/api/smarthome/deleteAll", b, function(a) {
            console.log("Purge ", a)
    ...
    f.getData = function(url) {   
        var c = d.defer();   
	return a.get(url+"/api/smarthome").then(function(a) {
	    //console.log(a);
	    f.data.response = a.data,
            ...
            b(f.getData(url), 2e3)
        },function(){console.log("Error occured when getting data.")}),
        c.promise
    }
    ,
    ...
    ,
    "/#/admin" === location.href.split(location.host)[1] && f.checkConnection(),
    setTimeout(function() {
        var url = "http://inteliotstorage.mybluemix.net";
        //Determine if the app is able to connect to cloud via GET request
        $.get("http://inteliotstorage.mybluemix.net/api/").then(function(data) {
            f.getData(url);
        }, function() {
            url = "http://192.168.1.1:3001";
            f.getData(url);
...
ctrls.controller("AdminController", ["$scope", "poller", "$http", "$timeout", function(a, b, c, d) {
    ...
    //Determine if the app is able to connect to cloud via GET request
    $.get("http://inteliotstorage.mybluemix.net/api/", function(data) {
        console.log( "reached cloud successfully" );
    })
        .done(function(){
            // add localhost path for the file socket.io.js in the index.html
        ...
        .fail(function() {
            console.log( "failed to reach cloud - OFFLINE mode" );
            url = "http://192.168.1.1:3001"
    ...
    var socketA;
    setTimeout(function() {
	//console.log("connect with websockets");
        //var serverDomainName = 'http://inteliotstorage.mybluemix.net/';
        socketA = io.connect(url, {reconnect: true});

        socketA.on("connect", function () {
            console.log("Smart Home Apps Connected to Server (Websockets)");
        });

        socketA.on("disconnect", function () {
            console.log("Smart Home Apps Disconnected from Server (Websockets)");
            if(serverDomainName == 'http://inteliotstorage.mybluemix.net/') {
                serverDomainName = 'http://localhost:3001';
    ...
    a.saveToJson = function(b) {
        ...
        d) {
        	if((c.name === "garage_door") && (c.status === "open" || c.status === "close")) {
                ...
                $.get("http://inteliotstorage.mybluemix.net/api/", function(data) {
                    console.log( "reached cloud successfully" );     
            	    $.post(url+"/api/smarthome", c, function(a) {}, "json").fail(function(a) {})
                })
                    .fail(function() {
                        console.log( "fail to reach cloud" );
                        url = "http://192.168.1.1:3001"
            	        $.post(url+"/api/smarthome", c, function(a) {}, "json").fail(function(a) {})
                    });
