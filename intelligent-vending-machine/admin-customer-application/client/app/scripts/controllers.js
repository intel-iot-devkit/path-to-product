...
ctrls.controller('AdminController', function ($scope, $cookies, poller, $http, $timeout, $route) {
   ...
   */
	$scope.$watch(
		...
		function(newVal, oldVal) {
    		...
    		//$scope.chargedTotal = 0;
    		//console.log('temperatureEvents',$scope.temperatureEvents);

    		if ($scope.productData.length === 0) {
    			$scope.loadDefaultProducts();
    		}


    		// start site first event
    		if ($scope.failureEvents !== undefined) {
    			//console.log('$scope.curMachineState',$scope.curMachineState);
    			if($scope.failureEvents.length === 0){
    				$scope.eventUpdate('door-closed');
    			}
    		}
  ...
  $scope.init = function() {
		...
		/* activate hasInternet var && sets PartitionKey SCope var */
		if(!$scope.isOffline()){
			$scope.PartitionKey = $cookies.get('PartitionKey');
			$('.init-start').addClass("hide");
			console.log('PartitionKey set to default '+$scope.PartitionKey);
		...
		if (location.href.split(location.host)[1] === '/#/admin'){
			...
			/* load machine into an init state */
			//console.log('$scope.curMachineState',$scope.curMachineState);
			//if ($scope.curMachineState !== 'closed'){
				//$scope.eventUpdate('door-closed');
				//$scope.curMachineState = 'closed';
			//}

	  	$scope.machineState('closed');
  ...
  /*
  * when a new mac address is used, new producst must be loaded.
   */
  $scope.loadDefaultProducts = function(){
  	...
  	console.log('Loading default products');
		...
		poller.updateProductData($scope.PartitionKey,1,tmpObj);
		...
		poller.updateProductData($scope.PartitionKey,2,tmpObj);
		...
		poller.updateProductData($scope.PartitionKey,3,tmpObj);
	...
	$scope.completeSale = function(step){
	    ...
	    switch(step) {
	        ...
	        case "receipt":
	        	...
	        	/*
	        	Does a fake update of the product count reduction.
...
poller.updateEventsData($scope.PartitionKey,'u14564255699062',tmpObj);*/
